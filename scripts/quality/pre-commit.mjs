...
if (files.length === 0) {
  console.log("[PASS] No staged files to check.");
  process.exit(0);
}
...
console.log(
  `[PASS] Pre-commit checks passed for ${files.length} staged file(s).`,
);
  ...
  for (const file of fileNames) {
    ...
    for (const pattern of patterns) {
      if (pattern.expression.test(content))
        throw new Error(`Possible ${pattern.name} in staged file: ${file}`);
    }
  }
}
...
function readCommands() {
  return JSON.parse(
    readFileSync(new URL("./commands.json", import.meta.url), "utf8"),
  );
}
...
function runConfigured(name, entries, context) {
  ...
  for (const entry of entries) {
    const targetFiles = filterFiles(context.files, entry.extensions);
    if (targetFiles.length === 0) {
      console.log(
        `[PASS] ${name}: no staged files match the configured extensions.`,
      );
      continue;
    }
    const args = entry.args
      .map((arg) => (arg === "{files}" ? targetFiles : arg))
      .flat();
...
function filterFiles(files, extensions) {
  if (!Array.isArray(extensions) || extensions.length === 0) return files;
...
function run(command, args) {
  ...
  const result = isWindowsPackageCommand
    ? spawnSync("cmd.exe", ["/d", "/s", "/c", `${command}.cmd`, ...args], {
        stdio: "inherit",
      })
